"""Paint the WhatsApp/X card: 1200x630, G centered in the square crop.

WhatsApp has two previews of the same JPEG:
- Compose: a square thumb (center crop).
- Sent: a wide ~1.91:1 banner.

Put the G in the middle so both stages keep the mark.
"""
...
LAYERS: Final[tuple[dict[str, float], ...]] = (
    {"mid": 0.30, "amp": 0.12, "k": 1.05, "w": 0.65, "alpha": 0.40, "line": 6.0},
    {"mid": 0.50, "amp": 0.15, "k": 1.55, "w": -0.9, "alpha": 0.70, "line": 8.0},
    {"mid": 0.70, "amp": 0.12, "k": 0.88, "w": 0.48, "alpha": 1.0, "line": 10.0},
)
T: Final[float] = 1.7
...
def draw_g(draw: ImageDraw.ImageDraw, width: int, height: int) -> None:
    """Open ring plus crossbar, centered in the 630px square WhatsApp crops."""
    scale: float = 13.8 * SCALE
...
def main() -> None:
    ...
    img.convert("RGB").save(OUT, "JPEG", quality=90, optimize=True, progressive=False)
